...
# Parsing arguments
parser = argparse.ArgumentParser(description='FFTNet reimplementation')
...
#%% Loading data
dset_train = YesNoDataset(csv_path='data/processed_yesno/train.csv', zpad_target_len=160000, transform=None) # TODO: feature-scaling
dset_test = YesNoDataset(csv_path='data/processed_yesno/test.csv', zpad_target_len=160000, transform=None)
...
def train(epoch):
    ...
    train_loss = 0.
    train_acc = []
    total_data_sz = len(train_loader.dataset.file_ids)
...
model = FFTNet().cuda() if USE_GPU else FFTNet().cpu()
